...
class RouletteWheelWagers:
    ...
    # Lower level methods for each specific bet, to be applied by the mapping methods above
    ...
    def place_colours_bet(self):
        ...
        while True:
            ...
            if bet_choice in colour_dict.keys():
                winnings = floor(self.stake / (self.wheel.colour_counts(colour_dict[bet_choice])/self.wheel.wheel_size()))
                confirmation = input(f"Confirm £{self.stake} stake on {colour_dict[bet_choice]}?\n"
                                     f"Winning this bet will return: £{winnings}"
                                     f"\n[Y]es, [N]o\n--->").upper()
                if confirmation == 'N':
                    continue
